...
export const Lexicon: React.FC = () => {
  return (
    <section
      id="lexicon"
      className="min-h-screen flex flex-col items-center justify-center px-6 py-20 bg-paper-dark border-b border-stone-300">
      <div className="max-w-6xl w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {skills.map((skill, idx) => (
            <div
              key={idx}
              className="group relative bg-paper p-8 border border-stone-300 shadow-sm hover:shadow-md transition-all hover:-translate-y-1">
              {/* Decorative corner */}
              ...
              <div className="flex flex-wrap gap-3">
                {skill.items.map((item, i) => (
                  <span
                    key={i}
                    className="font-sans text-sm bg-stone-100 text-stone-700 px-3 py-1 border border-stone-200 rounded-sm">
                    {item}
                  </span>
                ))}
              </div>
